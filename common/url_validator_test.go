...
func TestInitSessionCookieSettingsCanonicalizesAndConsumesTrustedOrigins(t *testing.T) {
	resetSessionCookieSettingsAfterTest(t)
	t.Setenv("SESSION_COOKIE_SECURE", "true")
	t.Setenv("SESSION_COOKIE_TRUSTED_URL", "https://EXAMPLE.com:443/, https://example.com")

	require.NoError(t, InitSessionCookieSettings())
	assert.Equal(t, []string{"https://example.com"}, SessionCookieTrustedURLs)
	assert.True(t, IsSessionCookieTrustedOrigin("https://example.com"))
	assert.True(t, IsSessionCookieTrustedOrigin("https://EXAMPLE.com:443"))
	assert.False(t, IsSessionCookieTrustedOrigin("http://example.com"))
	assert.False(t, IsSessionCookieTrustedOrigin("https://example.com.evil.test"))
	assert.False(t, IsSessionCookieTrustedOrigin("https://example.com/path"))
}
