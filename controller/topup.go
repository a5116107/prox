...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		topUp, quotaToAdd, credited, completeErr := model.RechargeEpay(verifyInfo.ServiceTradeNo, verifyInfo.Type)
		if completeErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 完成充值失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), completeErr.Error()))
			_, _ = c.Writer.Write([]byte("fail"))
			return
		}
		if credited {
