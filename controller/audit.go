...
var auditContentTemplates = map[string]string{
	...
	"agent.event_create":    "Created agent event ${event_type}",
	"agent.action_create":   "Created agent action ${action_type}",
	"agent.approval_decide": "Decided agent approval ${id} as ${decision}",
	"agent.risk_evaluate":   "Evaluated agent risk for user ${user_id} with score ${score}",

	"channel.create":             "Created channel ${name} (type ${type}, count ${count})",
	"channel.update":             "Updated channel ${name} (ID: ${id})",
	"channel.status_update":      "Changed channel ${name} (ID: ${id}) status from ${from} to ${to}",
