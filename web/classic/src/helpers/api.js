...
function getDynamicApiBaseURL() {
  ...
  if (!hostname.toLowerCase().startsWith('ai.')) return '';
  const portPart = port ? `:${port}` : '';
  return `${protocol}//api.${hostname.slice(3)}${portPart}`;
}
...
export async function getOAuthBindState(providerSlug) {
  if (!providerSlug) return '';
  const res = await API.get(`/api/oauth/${providerSlug}/bind/state`, {
...
async function prepareOAuthState(options = {}, providerSlug = '') {
  const { shouldLogout = false, bind = false } = options;
  if (bind) {
    return await getOAuthBindState(providerSlug);
  }
  if (shouldLogout) {
    try {
...
export async function onCustomOAuthClicked(provider, options = {}) {
  ...
  try {
    const isBindFlow = options?.bind === true;
    const redirect_uri = `${window.location.origin}/oauth/${provider.slug}${isBindFlow ? '?bind=true' : ''}`;
