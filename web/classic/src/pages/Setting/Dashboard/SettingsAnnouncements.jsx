...
const getDefaultCommunityFields = (
  communityType = SITE_DEFAULT_COMMUNITY.communityType,
) => {
  const type = String(communityType || SITE_DEFAULT_COMMUNITY.communityType)
    .toLowerCase()
    .startsWith('tg')
    ? 'tg'
    : 'qq';
...
const createDefaultAnnouncementForm = () => ({
  content: '',
...
const SettingsAnnouncements = ({ options, refresh }) => {
  ...
  const communityTypeOptions = [
    { value: 'qq', label: t('QQ群') },
    { value: 'tg', label: t('Telegram') },
  ];
  ...
  const normalizeCommunityType = (value) => {
    ...
    return ['tg', 'telegram'].includes(normalized) ? 'tg' : 'qq';
  };

  const getCommunityInfo = (record = {}) => {
    ...
    const value =
      record.communityValue ||
      record.community_value ||
      (type === 'tg'
        ? record.tgGroup || record.tg_group || record.telegram || ''
        : record.qqGroup ||
          record.qq_group ||
          extractQqGroup(record.content) ||
          '');
    const link =
      record.communityLink ||
      record.community_link ||
      (type === 'tg'
        ? record.tgGroupLink ||
          record.tg_group_link ||
          record.telegramLink ||
          record.telegram_link ||
          ''
        : record.qqGroupLink || record.qq_group_link || '');
  ...
  const columns = [
    ...
    {
      title: t('强制弹出'),
      dataIndex: 'forcePopup',
      key: 'forcePopup',
      width: 110,
      render: (value) => (
        <Tag color={value ? 'red' : 'grey'} shape='circle'>
          {value ? t('是') : t('否')}
        </Tag>
      ),
    },
    {
      title: t('社群入口'),
      dataIndex: 'communityValue',
      key: 'communityValue',
      width: 170,
      render: (text, record) => {
        const info = getCommunityInfo(record);
        return info.value || info.link ? (
          <Tag color={info.type === 'tg' ? 'blue' : 'cyan'} shape='circle'>
            {info.type === 'tg' ? 'TG' : 'QQ'} {info.value || t('已填链接')}
          </Tag>
        ) : (
          '-'
        );
  ...
  const handleEditAnnouncement = (announcement) => {
    ...
    setAnnouncementForm({
      ...
      forcePopup:
        announcement.forcePopup === true || announcement.force_popup === true,
      ...
      qqGroup:
        ...
        announcement.qq_group ||
        extractQqGroup(announcement.content) ||
        '',
      qqGroupLink: announcement.qqGroupLink || announcement.qq_group_link || '',
  ...
  const handleSaveAnnouncement = async () => {
    ...
    try {
      ...
      const communityType = normalizeCommunityType(
        announcementForm.communityType,
      );
      const defaultCommunity = getDefaultCommunityFields(communityType);
      const communityValue =
        (announcementForm.communityValue || '').trim() ||
        defaultCommunity.communityValue;
      const communityLink =
        (announcementForm.communityLink || '').trim() ||
        defaultCommunity.communityLink;
      const formData = {
        ...announcementForm,
        forcePopup: !!announcementForm.forcePopup,
        communityType,
        communityValue,
        communityLink,
        // 兼容旧前端与旧公告数据：QQ 仍同步写入旧字段，TG 使用新字段。
      ...
      >
        ...
        >
          ...
          <div className='flex items-center gap-2' style={{ marginBottom: 12 }}>
            <Switch
              checked={!!announcementForm.forcePopup}
              onChange={(checked) =>
                setAnnouncementForm({
                  ...announcementForm,
                  forcePopup: checked,
            ...
            />
            <Text>
              {t('强制弹出：忽略用户“今日关闭”，公告变更后会再次自动弹出')}
            </Text>
          </div>
          <Form.Select
            field='communityType'
            label={t('加群类型')}
            optionList={communityTypeOptions}
            prefix={
              announcementForm.communityType === 'tg' ? (
                <Send size={14} />
              ) : (
            ...
            onChange={(value) =>
              setAnnouncementForm({
                ...announcementForm,
                ...getDefaultCommunityFields(value),
              })
            }
          />
          <Form.Input
            ...
            placeholder={
              announcementForm.communityType === 'tg'
                ? t('例如 your_group 或 https://t.me/your_group')
                : t('例如 925249987；前台会自动生成一键加群按钮')
            }
            onChange={(value) =>
              setAnnouncementForm({
                ...announcementForm,
                communityValue: value,
                qqGroup: announcementForm.communityType === 'qq' ? value : '',
              })
            }
          />
          <Form.Input
            ...
            placeholder={
              announcementForm.communityType === 'tg'
                ? t(
                    '可选，例如 https://t.me/your_group；不填时会按上方内容生成',
                  )
                : t('可选，填写 qm.qq.com 链接时优先使用该链接')
            }
            onChange={(value) =>
              setAnnouncementForm({
                ...announcementForm,
                communityLink: value,
                qqGroupLink:
                  announcementForm.communityType === 'qq' ? value : '',
              })
            }
          />
          <Form.Input
