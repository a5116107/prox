...
function isForcePopupAnnouncement(item: Record<string, unknown>): boolean {
  return (
...
function getForcePopupSeenKeys(): string[] {
  try {
    return JSON.parse(
...
function markForcePopupSeen(key: string) {
  if (!key) return
