...
function OAuthCallback() {
  ...
  const isExplicitBind = search?.bind === 'true' || search?.bind === '1'
  const mode: 'login' | 'bind' = isExplicitBind ? 'bind' : 'login'
  ...
  useEffect(() => {
    ;(async () => {
      ...
      // Only an explicit bind=true callback is a binding flow.
      // Do not infer bind mode from window.opener: login/register pages can be
      // opened from another page and still must remain normal OAuth login.
      const isBindingFlow = isExplicitBind
      ...
      const finalizeLogin = async (): Promise<boolean> => {
        try {
          ...
          if (selfResponse?.success && selfResponse.data) {
            persistLoginUser(selfResponse.data)
            return true
          }
        } catch (_error) {
          void _error
      ...
      const persistLoginUser = (user: AuthUser | null | undefined) => {
        if (!user) return
      ...
      try {
        ...
        if (res?.data?.success) {
          ...
          if (message === 'bind' || res.data?.data?.action === 'bind') {
            if (isBindingFlow) {
            ...
            toast.error(
              i18next.t(
                'OAuth login returned a binding result; please restart sign in'
              )
            )
            safeNavigate('/sign-in')
            return
  ...
  }, [isExplicitBind, navigate, provider, search])
