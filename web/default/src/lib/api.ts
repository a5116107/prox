...
export async function getMyAccessControlStatus(params?: {
  refresh?: boolean
}): Promise<{
  success: boolean
  message?: string
  ...
  const res = await api.get('/api/access-control/me', {
    params: params?.refresh ? { refresh: true } : undefined,
  })
  return res.data
}

export async function refreshMyAccessControlStatus(): Promise<{
  success: boolean
  message?: string
  data?: {
  ...
  const res = await api.post('/api/access-control/refresh')
  return res.data
}
