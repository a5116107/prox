...
function isDirectNavigationUrl(url: unknown): boolean {
  if (typeof url !== 'string') return false
  return url.startsWith('/api/') || url.startsWith('/game-admin')
}
...
function SidebarMenuLink({ item, href }: { item: NavLink; href: string }) {
  ...
  const isDirectLink = isDirectNavigationUrl(item.url)

  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        isActive={!isDirectLink && checkIsActive(href, item)}
        tooltip={item.title}
        render={
          isDirectLink ? (
            <a href={itemUrl} onClick={() => setOpenMobile(false)} />
          ) : (
        ...
        <SidebarMenuSub>
          {item.items.map((subItem) => (
            <SidebarMenuSubItem key={subItem.title}>
              <SidebarMenuSubButton
                isActive={
                  !isDirectNavigationUrl(subItem.url) &&
