...
export function AccountBindingsTab({
  ...
  const handleBindCustomOAuth = async (provider: {
    ...
    if (
      ...
      toast.error(t('OAuth provider configuration is incomplete'))
      return
    }
    try {
      const stateRes = await api.get(`/api/oauth/${provider.slug}/bind/state`, {
        disableDuplicate: true,
      })
      const state = stateRes?.data?.data
      if (!stateRes?.data?.success || !state) {
        toast.error(
          stateRes?.data?.message || t('Failed to initialize OAuth binding')
        )
        return
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : t('Failed to initialize OAuth binding')
      toast.error(message)
    }
  }
          ...
          <div className='grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3'>
            {customProviders.map((provider) => {
              const binding = customBindings.find(
                (b) => Number(b.provider_id) === Number(provider.id)
              )
