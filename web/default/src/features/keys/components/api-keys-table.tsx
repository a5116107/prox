...
function hasAccessIssue(apiKey: ApiKey) {
  return (
    apiKey.access_control_frozen ||
    apiKey.community_gate_frozen ||
...
export function ApiKeysTable() {
  ...
  const hasProblemKey = apiKeys.some(hasAccessIssue)
  const accessSummary = getAccessControlDisplayMessage(accessControlStatus, t)
  const accessNextSteps = getAccessControlNextSteps(accessControlStatus, t)
  ...
  useEffect(() => {
    if (!hasProblemKey) return
    let cancelled = false
    ;(async () => {
      try {
        const res = await getMyAccessControlStatus()
        if (!cancelled) {
          setAccessControlStatus(res.data || null)
        }
      } catch {
        if (!cancelled) {
    ...
    <>
      {hasProblemKey ? (
        <div className='mb-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 text-sm'>
          <div className='font-medium'>{t('Some API keys require action')}</div>
          ...
          {accessNextSteps.length > 0 ? (
            ...
            <div className='text-muted-foreground mt-1'>
              {t(
                'Detected keys that were frozen by binding, community gate, or risk activation policies.'
              )}
            </div>
          )}
          {Array.isArray(accessControlStatus?.effective_groups) &&
          accessControlStatus.effective_groups.length > 0 ? (
            <div className='text-muted-foreground mt-2 text-xs'>
              {t('Currently allowed groups')}:{' '}
              {accessControlStatus.effective_groups.join(', ')}
            </div>
          ) : null}
