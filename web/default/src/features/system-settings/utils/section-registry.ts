...
> = {
  ...
  basePath: string
  aliases?: Partial<Record<string, TSectionId>>
...
>(config: SectionRegistryConfig<TSectionId, TSettings, TExtraArgs>) {
  ...
  function normalizeSectionId(sectionId: SectionId): SectionId {
    return (aliases[sectionId] ?? sectionId) as SectionId
  }
  ...
  function getSectionMeta(sectionId: SectionId) {
    const section =
      sections.find((item) => item.id === normalizeSectionId(sectionId)) ??
      sections[0]
    return section
  }
