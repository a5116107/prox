...
func normalizeUsernameLookupKey(raw string) string {
	return strings.ToLower(strings.TrimSpace(raw))
}

func trimUsernameToLength(raw string, limit int) string {
	if limit <= 0 {
		return ""
	}
	...
	return strings.Trim(strings.TrimSpace(string(runes)), "_-")
}
...
func sanitizeUsernameCandidate(raw string) string {
	...
	for _, r := range raw {
		...
		if written >= UserNameMaxLength {
			break
		}
	}
	return trimUsernameToLength(b.String(), UserNameMaxLength)
}

func BuildOAuthUsernameCandidate(prefix, stableID, preferred string) string {
	if preferredCandidate := sanitizeUsernameCandidate(preferred); preferredCandidate != "" {
		return preferredCandidate
	}
	prefix = strings.ToLower(strings.Trim(strings.TrimSpace(prefix), "_-"))
	prefix = sanitizeUsernameCandidate(prefix)
	if prefix == "" {
		prefix = "user"
	}
	suffix := strings.ToLower(sanitizeUsernameCandidate(stableID))
	if suffix == "" {
	...
	return trimUsernameToLength(base, UserNameMaxLength)
}

func assignAvailableUsernameWithTx(tx *gorm.DB, user *User) error {
	if tx == nil || user == nil {
		return errors.New("invalid username transaction")
	}
	requested := trimUsernameToLength(user.Username, UserNameMaxLength)
	if requested == "" {
		return nil
	}
	lookupKey := normalizeUsernameLookupKey(requested)
	switch {
	case common.UsingPostgreSQL:
		if err := tx.Exec("SELECT pg_advisory_xact_lock(hashtext(?))", "username:"+lookupKey).Error; err != nil {
			return err
		}
	case common.UsingMySQL:
		var ids []int
		if err := tx.Raw("SELECT id FROM users WHERE username = ? FOR UPDATE", requested).Scan(&ids).Error; err != nil {
			return err
		}
	}
	...
	available := func(candidate string) (bool, error) {
		...
		err := tx.Unscoped().Model(&User{}).
			Where("LOWER(username) = ?", normalizeUsernameLookupKey(candidate)).
			Count(&count).Error
		return count == 0, err
	}
	if ok, err := available(requested); err != nil {
		return err
	} else if ok {
	...
	for attempt := 0; attempt < 32; attempt++ {
		suffix := strings.ToLower(sanitizeUsernameCandidate(common.GetRandomString(6)))
		if suffix == "" {
			suffix = fmt.Sprintf("%06d", attempt+1)
		}
		base := trimUsernameToLength(requested, UserNameMaxLength-len([]rune(suffix))-1)
		candidate := trimUsernameToLength(base+"_"+suffix, UserNameMaxLength)
...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	idempotencyKey := fmt.Sprintf("invite-aff-transfer:%d:%d:%d", user.Id, user.AffQuota, quota)
	if err := EnsureUserReservedQuotaTx(
		tx,
		user.Id,
		"growth",
		quota,
		idempotencyKey+":reserve-repair",
		"补齐历史邀请额度冻结记录",
	); err != nil {
		return err
	}
	if err := ConsumeReservedQuotaFromBudgetPoolTx(tx, user.Id, "growth", quota, idempotencyKey, "兑现邀请奖励额度"); err != nil {
		return err
	}
...
func applyRegistrationGrowthBudgetTx(tx *gorm.DB, user *User, inviterId int) error {
	if tx == nil || user == nil || user.Id <= 0 {
		return errors.New("invalid registration budget context")
	}
	if common.QuotaForNewUser > 0 {
		key := fmt.Sprintf("signup:%d", user.Id)
		if err := GrantQuotaFromBudgetPoolWithSourceTx(tx, user.Id, "growth", common.QuotaForNewUser, key, "新用户注册赠额", "signup_reward", ""); err != nil {
			return err
		}
	}
	...
	if common.QuotaForInvitee > 0 {
		key := fmt.Sprintf("invitee:%d:%d", user.Id, inviterId)
		if err := GrantQuotaFromBudgetPoolWithSourceTx(tx, user.Id, "growth", common.QuotaForInvitee, key, "被邀请用户赠额", "invitee_reward", ""); err != nil {
			return err
		}
	}
	if common.QuotaForInviter > 0 {
		key := fmt.Sprintf("inviter-reserve:%d:%d", inviterId, user.Id)
		if err := ReserveQuotaFromBudgetPoolWithSourceTx(tx, inviterId, "growth", common.QuotaForInviter, key, "邀请人奖励冻结", "invite_inviter_reserve"); err != nil {
			return err
		}
...
func (user *User) prepareForInsert(tx *gorm.DB) error {
	...
	if err := assignAvailableUsernameWithTx(tx, user); err != nil {
		return err
	}
	if user.Password == "" {
		return nil
...
func (user *User) Insert(inviterId int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			return applyRegistrationGrowthBudgetTx(tx, user, inviterId)
		})
	}); err != nil {
		return err
	}
