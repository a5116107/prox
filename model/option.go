...
func applyOptionsToRuntime(values map[string]string) error {
	for k, v := range values {
...
// UpdateOptionsBulkAtomically commits option writes and an optional related
// mutation in one transaction. Runtime settings are refreshed only after the
// database transaction commits.
func UpdateOptionsBulkAtomically(values map[string]string, mutate func(tx *gorm.DB) error) error {
	if len(values) == 0 {
		if mutate == nil {
			return nil
		}
	...
	return applyOptionsToRuntime(values)
}

// UpdateOptionsBulk persists multiple related options atomically.
