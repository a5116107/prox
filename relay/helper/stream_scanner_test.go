...
func TestStreamingTimeoutDuration(t *testing.T) {
	t.Parallel()

	assert.Equal(t, DefaultStreamingTimeout, streamingTimeoutDuration(0))
	assert.Equal(t, DefaultStreamingTimeout, streamingTimeoutDuration(-1))
	assert.Equal(t, 30*time.Second, streamingTimeoutDuration(30))
}
...
func TestStreamScannerHandler_StreamStatus_HandlerStop(t *testing.T) {
	...
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for handler stop")
	}
